  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const line_items = cartItems.map((item: any) => {
        ...
        if (item.images) {
          itemImages = Array.isArray(item.images) ? item.images : [item.images];
        }
...
app.get('/api/details/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (!ObjectId.isValid(id)) {
      return res.status(400).json({ message: 'Invalid Products ID format' });
    }
...
// update Quantity
app.patch('/api/cart/update-quantity', async (req: Request, res: Response) => {
  try {
